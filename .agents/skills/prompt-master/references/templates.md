...
| [N — Music AI](#template-n--music-ai) | Suno, Udio, ElevenLabs Music — AI music generation |
| [O — Context Engineering System](#template-o--context-engineering-system) | Production pipelines, multi-turn agents, RAG systems |
...
*For Suno, Udio, and ElevenLabs Music. A music AI prompt has two parts: the style prompt and the lyrics.*

### Suno Format
```
STYLE PROMPT (comma-separated tags — most important first):
[primary genre], [sub-genre], [mood], [vocal type], [instrumentation], [production style], [tempo descriptor]

LYRICS (with structural metatags):
[Verse 1]
[lyrics...]
...
```
STYLE PROMPT:
cinematic pop, orchestral synth, ethereal, female soprano, layered strings, reverb-heavy, slow build

LYRICS:
[Verse 1]
We were golden in the half-light
Chasing echoes through the rain

[Chorus]
Hold on, hold on — the world is spinning
But we are still, we are beginning
...
### Udio Format
```
DESCRIPTION (prose, detailed — Udio accepts longer prompts):
[Detailed description of the desired sound, genre, instrumentation, vocal quality, production style, and mood. Be specific about sonic textures and references.]

NEGATIVE PROMPT (what to AVOID — Udio-specific):
[Elements to exclude: distortion, autotune, spoken word, etc.]
```

### ElevenLabs Music Format
```
GENRE: [primary genre]
MOOD: [emotional tone]
INSTRUMENTATION: [specific instruments and layers]
TEMPO: [BPM or descriptor]
USE CASE: [background music / soundtrack / game score / meditation]
```

**Key rules:**
- Front-load the most important genre/mood tags — truncation is silent on older Suno models (~200 chars)
- Suno V5 allows up to 1000 chars in the style prompt
- Each comma-separated term acts as a weighted tag — order matters
- Without `[Verse]`/`[Chorus]` metatags, Suno drifts structurally
- For Udio: use the negative prompt to exclude unwanted elements explicitly

**When to use:** Any music AI generation task. Detect the platform (Suno, Udio, ElevenLabs) and use the matching format.

---

## Template O — Context Engineering System

*For production pipelines, multi-turn agents, RAG systems, and API-level deployments where the full context window architecture matters more than a single prompt.*

```
┌─────────────────────────────────────────────────────┐
│  STATIC PREFIX (cacheable — changes rarely)          │
│  ┌───────────────────────────────────────────────┐   │
...
│  - Claude: new session + cache_control + task budget │
│  - Gemini: pass thoughtSignature between turns       │
├─────────────────────────────────────────────────────┤
│  RETRIEVAL BUDGET (GPT-5.5)                          │
│  "Search at most [N] sources before answering."      │
│  "Cite every factual claim with [source:page]."      │
├─────────────────────────────────────────────────────┤
│  VALIDATION LOOP                                     │
│  "Before finalizing, verify:"                        │
│  1. All constraints from the static prefix are met   │
│  2. Output matches the format specification          │
│  3. No hallucinated facts or citations               │
└─────────────────────────────────────────────────────┘
```

**Design principles:**
- Separate static (cacheable) from dynamic (per-call) context
- Optimal density > volume — compact ruthlessly, remove redundant context
- Place instructions at start and end of window (highest attention)
- Place data in the middle (weakest attention, but anchored by instructions)
- Pin model version for production stability (e.g., `gpt-5-2025-08-07`)
- Set `reasoning_effort` / `thinking_level` via API, not via prompt text

**When to use:** User is building a production API pipeline, multi-turn agent, RAG system, or any system where the context window architecture matters. Not needed for single-turn chat interactions.
